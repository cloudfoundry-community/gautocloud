...
var _ = Describe("Loader", func() {
	...
	Context("GetFirst", func() {
		...
		Context("connector give a pointer to a structure", func() {
			...
			It("should return a correct content given by connector", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(data).Should(BeEquivalentTo(&srv1Expected))
				Expect(reflect.TypeOf(data).Kind()).Should(Equal(reflect.Ptr)) //nolint:govet
			})
		})
	...
	Context("GetAll", func() {
		...
		Context("connector give a pointer to a structure", func() {
			...
			Expect(reflect.TypeOf(data[0]).Kind()).Should(Equal(reflect.Ptr)) //nolint:govet
			Expect(reflect.TypeOf(data[1]).Kind()).Should(Equal(reflect.Ptr)) //nolint:govet
			})
		})

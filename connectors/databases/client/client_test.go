		...
		Context("GetConnString", func() {
			...
			Context("When there is no password given", func() {
				It("should return correct connection string", func() {
					...
					Expect(connString).Should(ContainSubstring("user@tcp(localhost:3306)/db"))
					Expect(connString).Should(ContainSubstring("tls=true"))
					Expect(connString).Should(ContainSubstring("parseTime=true"))
				})
			})
			...
			Context("When there is password and options given", func() {
				It("should return correct connection string with filtered options", func() {
					connString := mysqlConnector.GetConnString(dbtype.MysqlDatabase{
						Host:     "localhost",
						Port:     3306,
						User:     "user",
						Password: "pass",
						Options:  "tls=true&option=1",
						Database: "db",
					})
					Expect(connString).Should(ContainSubstring("user:pass@tcp(localhost:3306)/db"))
					Expect(connString).Should(ContainSubstring("tls=true"))
					Expect(connString).Should(ContainSubstring("parseTime=true"))
					Expect(connString).ShouldNot(ContainSubstring("option=1"))
				})
			})
		})
	})
